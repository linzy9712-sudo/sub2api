		...
		if cached, ok := r.cache.Get(key); ok {
			if multiplier, castOK := cached.(float64); castOK {
				...
				if billingguard.ShouldLogMultiplier(multiplier) {
					if emit, suppressed := billingguard.LogThrottle("resolver:" + key); emit {
						attrs := []any{
						...
						slog.Warn(billingguard.LogTag+"suspicious user group rate (resolver cache hit)", attrs...)
					}
				}
				return multiplier
			}
		...
		if userRate != nil {
			...
			if billingguard.ShouldLogMultiplier(multiplier) {
				if emit, suppressed := billingguard.LogThrottle("resolver:" + key); emit {
					...
					slog.Warn(billingguard.LogTag+"suspicious user group rate (resolver db load)", attrs...)
				}
			}
		}
		if r.cache != nil {
